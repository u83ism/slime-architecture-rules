...
- 本ドキュメントは**FW非依存のアーキテクチャ論**として成立するよう設計されています。SlimeというFWがこの設計論を実装・強制するために作られていますが、本ドキュメントの内容はSlimeなしでも任意のFW上で手動実装できます。
  - SlimeおよびKaachanによる具体的な実装・自動検知・強制の詳細は[FW文書](./🤤%20僕の考えた最強の次世代Webアプリケーションフレームワーク（案）.md)を参照してください
...
> 💧 **Kaachan/Slime**: 上記ルールの自動検知・Slimeが提供するauth等の公式Middlewareについては[FW文書](./🤤%20僕の考えた最強の次世代Webアプリケーションフレームワーク（案）.md#lv-2-1)参照
...
> 💧 **Kaachan/Slime**: 自動検知・`slime export:schema` / `slime export:openapi`の詳細は[FW文書](./🤤%20僕の考えた最強の次世代Webアプリケーションフレームワーク（案）.md#lv-3-1)参照
...
> 💡 **ADR**: [なぜWorkflowはHTTPに依存しない設計になっているか](./Kaachan%26Slime%26Slime%20Architecture構想の設計根拠、補足資料.md#adr-lv3なぜworkflowはhttpに依存しない設計になっているか)
...
> 💧 **Kaachan/Slime**: 自動検知・`withTransaction()`の実装については[FW文書](./🤤%20僕の考えた最強の次世代Webアプリケーションフレームワーク（案）.md#lv-4-1)参照
...
> 💧 **Kaachan/Slime**: 肥大化の自動検知閾値・エスカレーション（Hint/Warning/Error）・Result型の提供については[FW文書](./🤤%20僕の考えた最強の次世代Webアプリケーションフレームワーク（案）.md#lv-5-1)参照
>
> 💡 **ADR**: [なぜ「Logicだけ」Result型なのか（ROPの射程）](./Kaachan%26Slime%26Slime%20Architecture構想の設計根拠、補足資料.md#adr-lv5なぜlogicだけresult型なのかropの射程) / [ResultAsync（チェーン専用型）を採用しない理由](./Kaachan%26Slime%26Slime%20Architecture構想の設計根拠、補足資料.md#adr-lv5resultasyncチェーン専用型を採用しない理由)
...
- App
  ...
  - **app/route.ts はエントリーポイントのみ**（domainルートをimportして束ねる）
...
> 💧 **Kaachan/Slime**: 自動検知の詳細は[FW文書](./🤤%20僕の考えた最強の次世代Webアプリケーションフレームワーク（案）.md#lv-6-1)参照
...
> 💧 **Kaachan/Slime**: `defer()` の提供・自動検知の詳細は[FW文書](./🤤%20僕の考えた最強の次世代Webアプリケーションフレームワーク（案）.md#lv-7-1)参照
...
> 💧 **Kaachan/Slime**: 自動検知の詳細は[FW文書](./🤤%20僕の考えた最強の次世代Webアプリケーションフレームワーク（案）.md#lv-8-1)参照
...
> 💡 **ADR**: [なぜイベントバス（emit/on）ではなく戻り値方式を選んだか](./Kaachan%26Slime%26Slime%20Architecture構想の設計根拠、補足資料.md#adr-lv8なぜイベントバスemitonではなく戻り値方式を選んだか)
...
> 💧 **Kaachan/Slime**: 自動検知の詳細は[FW文書](./🤤%20僕の考えた最強の次世代Webアプリケーションフレームワーク（案）.md#lv-9-1)参照
>
> 💡 **ADR**: [storeという名前の来歴と分離](./Kaachan%26Slime%26Slime%20Architecture構想の設計根拠、補足資料.md#adr-lv9storeという名前の来歴と分離) / [なぜPorts & AdaptersにDIコンテナを使わないか](./Kaachan%26Slime%26Slime%20Architecture構想の設計根拠、補足資料.md#adr-lv9なぜports--adaptersにdiコンテナを使わないか)
...
>
> 💡 **ADR**: [なぜCQRSはLv10なのか（Lv6でよいのでは）](./Kaachan%26Slime%26Slime%20Architecture構想の設計根拠、補足資料.md#adr-lv10なぜcqrsはlv10なのかlv6でよいのでは)
