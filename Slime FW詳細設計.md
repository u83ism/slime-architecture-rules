...
Slime Architectureは「DBアクセスが必ずrepository.tsを通る」「外部APIが必ずclient.tsを通る」「ユースケースが必ずWorkflowを通る」という構造をFWが強制しているため、各境界でFWが透過的にラップすることで、ユーザーが計測コードを一切書かずにメトリクスを取れる可能性がある。Laravelのクエリロギングと同じ発想だが、DB以外の境界（外部API・Workflow）もカバーできる点がSlimeの強み。
...
- **Lv4**：Repository/Clientが分離されるタイミングでこれらも自動計装対象に加わる
- **Lv6以降**：ドメイン別のWorkflow/Repositoryが分かれるので、ドメイン別のメトリクスが自然に取れるようになる
